        ...
        \begin{center}
          \begin{tikzpicture}
            \begin{axis}[
              ...
              {
                % Hack to make these auto-calaculated boxplot values available
                % globally. Can't use \pgfplotsetmacro since it's local. Can't
                % use \global with \pgfplotsetmacro. Can't use \boxplotvalue
                % with \let. Thus, \pgfmathparse first puts the result in
                % \pgfmathresult, which CAN be used with \let. Very cool.
                \pgfmathparse{\boxplotvalue{lower whisker}}%
                \global\let\dataonelw\pgfmathresult%
                \pgfmathprintnumber{\dataonelw}
              }
              node at
              (boxplot box cs: \boxplotvalue{lower quartile},1)
              {
                \pgfmathparse{\boxplotvalue{lower quartile}}%
                \global\let\dataonelq\pgfmathresult%
                \pgfmathprintnumber{\dataonelq}
              }
              node[left] at
              (boxplot box cs: \boxplotvalue{median},0.5)
              {
                \pgfmathparse{\boxplotvalue{median}}%
                \global\let\dataonemedian\pgfmathresult%
                \pgfmathprintnumber{\dataonemedian}
              }
              node at
              (boxplot box cs: \boxplotvalue{upper quartile},1)
              {
                \pgfmathparse{\boxplotvalue{upper quartile}}%
                \global\let\dataoneuq\pgfmathresult%
                \pgfmathprintnumber{\dataoneuq}
              }
              node at
              (boxplot box cs: \boxplotvalue{upper whisker},1)
              {
                \pgfmathparse{\boxplotvalue{whisker range}}%
                \global\let\dataonewr\pgfmathresult%
                \pgfmathparse{\boxplotvalue{upper whisker}}%
                \global\let\dataoneuw\pgfmathresult%
                \pgfmathprintnumber{\dataoneuw}
              };
            \end{axis}
          \end{tikzpicture}
        \end{center}

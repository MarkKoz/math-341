  ...
  \item[2.]
    ...
    \begin{enumerate}
          ...
          \begin{tikzpicture}
            \begin{axis}[
              ...
              \addplot+[
                ...
                y filter/.expression = {y / \rowcount},
              ] table[y index = 0] {\datatwo};
            \end{axis}
          \end{tikzpicture}
        \end{center}
      \item[c.]
      ...
      \item[e--g.]
        ...
        \begin{center}
          \boxplot{\datatwo}{datatwo}{width = .9\linewidth, xtick distance = 6}{}
        \end{center}
        The quartiles are calculated using the formula from the \texttt{R7} estimator of \texttt{pgfplots}.
        \begin{align*}
          h_1 &= \frac{n - 1}{4} + 1 \\
        ...
        \begin{align*}
          \text{IQR} &= Q_3 - Q_1 \\
          &= \datatwouq - \datatwolq \\
          &= \eval{\datatwouq - \datatwolq}
        \end{align*}
        Along with the whisker range of $\datatwowr$, the IQR is used to determine the lower and upper whiskers.
        \begin{align*}
          \text{lower whisker} &= Q_1 - \datatwowr \cdot IQR \\
          &= \datatwolq - \datatwowr \cdot \eval{\datatwouq - \datatwolq} \\
          &= \eval{\datatwolq - \datatwowr * (\datatwouq - \datatwolq)} \\
          \text{upper whisker} &= Q_3 + \datatwowr \cdot IQR \\
          &= \datatwouq + \datatwowr \cdot \eval{\datatwouq - \datatwolq} \\
          &= \eval{\datatwouq + \datatwowr * (\datatwouq - \datatwolq)}
        \end{align*}
        Any observation less than the lower whisker or greater than the upper whisker is an outlier. The whisker values are normalised to the nearest non-outlier value.
        \\ \\
        The plot does not show symmetry; it is slightly skewed to the right. There is a significant amount of mild outliers, but no extreme outliers. The IQR is relatively small, so there is low variability.
    \end{enumerate}
  \item[3.]
    ...
    \begin{enumerate}
      \item[1.]
        ...
        \begin{center}
          \begin{tblr}{colspec = >{\itshape}l *{3}c}
            ...
            \hline
            Class              &  60--<64 &   64--<68 &   68--<72 \\
            Frequency          &       20 &        21 &        13 \\
            Relative frequency &     .055 &      .058 &      .036 \\
            \hline
            Class              &  72--<76 &   76--<80 &   80--<84 \\
            Frequency          &       25 &        15 &        12 \\
            Relative frequency &     .068 &      .041 &      .033 \\
            \hline
            Class              &  84--<88 &   88--<92 &   92--<96 \\
            Frequency          &       18 &        33 &        34 \\
            Relative frequency &     .049 &      .090 &      .093 \\
            \hline
            Class              & 96--<100 & 100--<104 & 104--<108 \\
            Frequency          &       12 &         9 &         1 \\
            Relative frequency &     .033 &      .025 &      .003
          \end{tblr}
        \end{center}
        \sd{\datathree}
        \calcrange{\datathree}
        \bininterval
        The frequency is the count of the observations which fall within the interval. The relative frequency is the ratio of the frequency to the sample size ($\rowcount$). For example, the class interval 24--<28 has 5 observations $\{24, 25, 25, 26, 27\}$, so the frequency is 5 and the relative frequency is $\frac{5}{\rowcount} \approx \evalp{5 / \rowcount}{3}$.
      \item[2.]
        ...
        \begin{center}
          \begin{tikzpicture}
            \begin{axis}[
              ...
              ylabel = {Relative Frequency},
              every x tick label/.append style={font=\fontsize{6}{6}},
            ]
              \addplot+[
                hist = {
                  bins = 21,
      ...
      \item[3.]
        Determine the sample mean, median, and mode.
        % Sum is too large without fpu, so hard-code it.
        \pgfplotstablegetelem{0}{mean}\of{\datathree}%
      ...
      \item[6--8.]
        ...
        Determine the 5-number-summary, draw the corresponding box plot, and comment on interesting characteristics (symmetry, outliers, variability and so on).
        \begin{center}
          \boxplot{\datathree}{datathree}{width = \linewidth, xtick distance = 8}{}
        \end{center}
        The quartiles are calculated using the formula from the \texttt{R7} estimator of \texttt{pgfplots}.
        \begin{align*}
          h_1 &= \frac{n - 1}{4} + 1 \\
